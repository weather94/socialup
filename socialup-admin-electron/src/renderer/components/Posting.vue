<template>
  <div class="Firends">
    ...
    <md-radio v-model="postView" value="repeat">유령계정 포스팅 갈기기</md-radio>
...
import GhostPosting from '@/assets/js/GhostPosting';
...
export default {
  ...
  methods: {
    changePath(event) {
      this.imageFolderPath = (event.target.files[0]) ? event.target.files[0].path : null;
    },
    deletePostingLog() {
    ...
    async postingRepeatToggle() {
      if (this.ghostPostingWorker) {
        this.ghostPostingWorker();
        this.ghostPostingWorker = null;
      } else {
        this.ghostPostingWorker = GhostPosting(this.$http, this.imageFolderPath, this.$store.state.posting.repeat.hashtags);
      }
    },
    removePosting() {
