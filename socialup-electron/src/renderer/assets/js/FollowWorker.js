...
async function loadCookie(store, driver, cookies, id) {
  try {
    if (cookies) {
      ...
      if (cookies.sessionStorage) {
        for (const item of cookies.sessionStorage) {
          await driver.executeScript(`sessionStorage['${item.key}'] = '${item.value}';`);
        }
      }
      if (cookies.localStorage) {
        for (const item of cookies.localStorage) {
...
function messageLog(store, message, id) {
  store.dispatch('addInstagramLog',  new FollowLog(20, false, '언팔로우', message, null, null, id));
}
...
async function alertAndClose(store, driver, message, id) {
  ...
  store.dispatch('addWorkLog', new WorkLog(5, 'success', id, 'fw', 'alert_close'));
  messageLog(store, message, id);
}
...
async function submitComment2(store, driver, comment, account) {
  try {
    ...
    await sleep(store, 1, account.id);
    await driver.wait(until.elementLocated(By.xpath(`//a[@title="${account.username}"]`)), 15000);
  } catch(e) { console.log(e); }
  return true;
}

async function followLikeComment(store, driver, command, setting, account, hashtag, data) {
  ...
  if (isFollow) {
    ...
    await rsleep(store, 2, 4, account.id);
  }
  ...
  if (isComment) {
    comment = getComment(store, setting, data, account.id);
    await submitComment2(store, driver, comment, account);
    await rsleep(store, 2, 4, account.id);
  }
  ...
  if (isComment) {
    try {
      await driver.wait(until.elementLocated(By.xpath(`//a[@title="${account.username}"]`)), 1000);
      store.dispatch('addInstagramLog', new FollowLog(3, false, '댓글', comment, user, postUrl, account.id));
    } catch (e) {
      console.log(e);
      console.log(`${account.id} 코멘트 누락`);
...
async function clickLike(store, driver, user, postUrl, id) {
  try {
    ...
    store.dispatch('addInstagramLog', new FollowLog(2, true, '좋아요', '', user, postUrl, id));
...
async function submitComment(store, driver, user, postUrl, comment, username, id) {
  try {
    ...
    try {
      ...
      await store.dispatch('addInstagramLog',  new FollowLog(3, true, '댓글', comment, user, postUrl, id));
    }
            ...
            if (followings && followings[index] && followings[index].user) {
              ...
              try {
                ...
                if (data.follows_viewer === false && data.followed_by_viewer) {
                  ...
                  store.dispatch('addInstagramLog', new FollowLog(5, false, '맞팔로우', '확인', followings[index].user, '맞팔로우', account.id));
                }
              } catch (e) {
...
async function checkFriends(store, driver, id) {
  ...
  if (!store.state.instagram.friends[id] || standard > new Date(store.state.instagram.friends[id].date)) {
    messageLog(store, '친구목록을 불러오는 중 입니다', id);
...
function checkOmit(store, account) {
  const date = new Date().toLocaleDateString();
  if (store.state.instagram.logs[account.id][date]
    && (store.state.instagram.logs[account.id][date].followOmit > 10
    || store.state.instagram.logs[account.id][date].likeOmit > 1
    || store.state.instagram.logs[account.id][date].commentOmit > 20)) {
      store.dispatch('addWorkLog', new WorkLog(110, 'error', account.id, 'check_omit', '인스타그램 기능정지'));
    return true;
  } else {
    return false;
  }
...
function run(store, http, process, account, setting) {
  ...
  setTimeout(async () => {
    await store.dispatch('addInstagramLog',  new FollowLog(20, false, '언팔로우', '작업을 시작합니다', null, null, account.id));
          ...
          if (checkActiveTime(store, setting.activeTimes, account.id)) {
            ...
            if (new Date().getTime() - beforeTime >= CHECK_TIME) {
              ...
              if (await checkToken(store, http, process.token, account.id)) {
                alertAndClose(store, driver, '중복 로그인으로 작업을 종료합니다.', account.id);
                break;
              }
            ...
            if (setting.followType === 1) {
              if(setting.targetTags.length < 1) {
                messageLog(store, '타겟태그가 없어 작업을 종료합니다', account.id);
                break;
              }
              for (let hashtag of setting.targetTags) {
                ...
                // 활동시간 체크 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                if (checkActiveTime(store, setting.activeTimes, account.id)) {
                  if (checkFollowLimit(store, setting, account.id) || checkLikeLimit(store, setting, account.id) || checkCommentLimit(store, setting, account.id)) {
                    if (!driver) {
                      driver = await new Builder().forBrowser('chrome').withCapabilities(chromeCapabilities).build();
                      await driver.get('https://www.instagram.com/?hl=ko');
                      await loadCookie(store, driver, account.cookie, account.id);
                        ...
                        if (checkProcessingProbability(store, setting, account.id) && await spamCheck(store, setting, data, account.id)) {
                          ...
                          if (checkFollowLimit(store, setting, account.id) && checkLikeLimit(store, setting, account.id) && checkCommentLimit(store, setting, account.id)) {
                            if (data.owner.followed_by_viewer === false && data.viewer_has_liked === false && data.comments_disabled === false) {
                              await followLikeComment(store, driver, 'flc', setting, account, hashtag, data);
                              await sleep(store, setting.processingInterval / 2, account.id);
                              doWork = true;
                            }
                          } else if (checkFollowLimit(store, setting, account.id) && checkLikeLimit(store, setting, account.id)) {
                            if (data.owner.followed_by_viewer === false && data.viewer_has_liked === false) {
                              await followLikeComment(store, driver, 'fl', setting, account, hashtag, data);
                              await sleep(store, setting.processingInterval / 2, account.id);
                              doWork = true;
                            }
                          } else if (checkFollowLimit(store, setting, account.id) && checkCommentLimit(store, setting, account.id)) {
                            if (data.owner.followed_by_viewer === false && data.comments_disabled === false) {
                              await followLikeComment(store, driver, 'fc', setting, account, hashtag, data);
                              await sleep(store, setting.processingInterval / 2, account.id);
                              doWork = true;
                            }
                          } else if (checkLikeLimit(store, setting, account.id) && checkCommentLimit(store, setting, account.id)) {
                            if (data.viewer_has_liked === false && data.comments_disabled === false) {
                              await followLikeComment(store, driver, 'lc', setting, account, hashtag, data);
                              await sleep(store, setting.processingInterval / 2, account.id);
                              doWork = true;
                            }
                          } else if (checkFollowLimit(store, setting, account.id)) {
                            if (data.owner.followed_by_viewer === false) {
                              await followLikeComment(store, driver, 'f', setting, account, hashtag, data);
                              await sleep(store, setting.processingInterval / 2, account.id);
                              doWork = true;
                            }
                          } else if (checkLikeLimit(store, setting, account.id)) {
                            if (data.viewer_has_liked === false) {
                              await followLikeComment(store, driver, 'l', setting, account, hashtag, data);
                              await sleep(store, setting.processingInterval / 2, account.id);
                              doWork = true;
                            }
                          } else if (checkCommentLimit(store, setting, account.id)) {
                            if (data.comments_disabled === false) {
                              await followLikeComment(store, driver, 'c', setting, account, hashtag, data);
                              await sleep(store, setting.processingInterval / 2, account.id);
                              doWork = true;
                            }
                          }
                          ...
                          await sleep(store, 2, account.id);
                        }
                      } else {
                        await saveCookie(store, driver, account.id);
                        try { await driver.quit(); driver = null; } catch(e) { }
                        break;
                      }
                    }
                  } else {
              ...
              if (setting.followerOrder === 1) {
                followers = followers.filter(item => item.follower === true && item.following === true);
              } else if (setting.followerOrder === 2) {
                followers = followers.filter(item => item.follower === true);
              } else if (setting.followerOrder === 3) {
                followers = followers.filter(item => item.following === true);
              } else if (setting.followerOrder === 4) {
                followers = followers.filter(item => item.follower === true && item.following === false);
              } else if (setting.followerOrder === 5) {
                followers = followers.filter(item => item.follower === false && item.following === true);
              }
              ...
              for (const follower of followers) {
                ...
                if (checkActiveTime(store, setting.activeTimes, account.id)) {
                  if (checkLikeLimit(store, setting, account.id) || checkCommentLimit(store, setting, account.id)) {
                    try {
                      ...
                      if (data.followed_by_viewer === true && data.edge_owner_to_timeline_media.edges.length > 0) {
                        ...
                        for (const edge of data.edge_owner_to_timeline_media.edges) {
                          ...
                          if (index <= setting.likePerFollower && post.viewer_has_liked === false && checkLikeLimit(store, setting, account.id)) {
                            await clickLike(store, driver, user, postUrl, account.id);
                            await sleep(store, 1, account.id);
                          }
                          if (index <= setting.commentPerFollower && post.comments_disabled === false && checkCommentLimit(store, setting, account.id)) {
                            try {
                              await driver.wait(until.elementLocated(By.xpath(`//a[@title="${account.username}"]`)), 10000);
                            } catch (e) {
              ...
              if (followers.length <= 0) {
                messageLog(store, '언팔할 유저가 없어 작업을 종료합니다', account.id);
                break;
              }
              let index = 1;
              for (const follower of followers) {
                  ...
                  if (checkUnfollowLimit(store, setting, account.id)) {
                    try {
                      ...
                      if (data.followed_by_viewer) {
                        await clickUnfollow(store, driver, follower.name, `${tag} (${index++}/${followers.length})`, account.id);
                        await sleep(store, setting.processingInterval, account.id);
                      } else {
    ...
    // 로그인실패 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      alertAndClose(store, driver, '인스타그램 로그인실패로 작업을 종료', account.id);
    }
  ...
  return () => {
    if (driver) {
      ...
      messageLog(store, '작업을 종료 합니다.', account.id);
